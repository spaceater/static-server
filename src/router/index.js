...
const { handleImage, supported_image_extensions } = require("../handler/imageHandler");
...
function registerRoutes(app) {
  app.get("/*", async (req, res) => {
    try {
      ...
      if (isImageFile(file_path, supported_image_extensions)) {
        if (!(await handleImage(file_path, req, res))) {
          res.status(404).send("ERROR: " + file_path + " not found.");
        }
      } else {
        if (!(await handleFile(file_path, res))) {
          res.status(404).send("ERROR: " + file_path + " not found.");
        }
      }
    } catch (err) {
