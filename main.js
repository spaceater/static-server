...
//获取人物图片数量
app.get("/api/figures_sum", async (req, res) => {
    //禁用强缓存和协商缓存，page_view必须从服务器获取
    res.setHeader("Cache-Control", "no-store, no-cache, must-revalidate, proxy-revalidate");
    res.setHeader("Pragma", "no-cache");
    res.setHeader("Expires", "0");
    try {
        fs.readdir("./resources/images/figures",(err,files)=>{
            res.json({"figures_sum" : files.length});
        });
    } catch (err) {
        res.json({"figures_sum" : 0});
    }
});
...
app.get("/assets/image/:path(*)", (req, res) => {
    ...
    try{
        ...
        for (const extension of supported_extensions) {
            image_path = "./resources/images/" + req.params["path"] + extension;
...
app.get("/",(req,res)=>{
    ...
    fs.createReadStream("./resources/home.html").pipe(res);
});

//2024.12概率论与数理统计-区间估计
app.get("/interval-estimate",(req,res)=>{
    res.writeHead(200, {"Content-Type":"text/html;charset=utf-8"});
    fs.createReadStream("./resources/interval-estimate.html").pipe(res);
});

//图标匹配
app.get("/favicon.ico",(req,res)=>{
    res.sendFile(path.join(__dirname, "/resources/images/favicon.ico"));
});
